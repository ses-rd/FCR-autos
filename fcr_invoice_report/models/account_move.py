        ...
        for record in self:
            ...
            if tax_totals.get('has_tax_groups', False):
                ...
                for subtotal in subtotals:
                    ...
                    tax_group_iva13 = self.env['account.tax.group'].search([('l10n_do_billing_indicator', 'in', ['taxable_itbis', 'taxable_isr', 'tips', 'other'])], limit=1)

                    # for tax_group in tax_groups:
                    for tax_group in list(filter(lambda m: m['id'] == tax_group_iva13.id, tax_groups)):
