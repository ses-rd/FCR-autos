    ...
    def verify(
        ...
        **deprecated_kwargs,
        ...
        if x509_data is not None or self.config.require_x509:
            ...
            try:
                # Use cryptography APIs instead of deprecated OpenSSL.crypto.verify()
                cert_crypto = signing_cert.to_cryptography()
                pubkey = cert_crypto.public_key()
                digest_alg_impl = digest_algorithm_implementations[signature_alg]()
                
                if isinstance(pubkey, rsa.RSAPublicKey):
                    if signature_alg.name.startswith("RSA_"):
                        pubkey.verify(raw_signature, signed_info_c14n, PKCS1v15(), digest_alg_impl)
                    else:
                        pubkey.verify(
                            raw_signature,
                            signed_info_c14n,
                            PSS(mgf=MGF1(digest_alg_impl), salt_length=digest_alg_impl.digest_size),
                            digest_alg_impl,
                        )
                elif isinstance(pubkey, ec.EllipticCurvePublicKey):
                    pubkey.verify(raw_signature, signed_info_c14n, ec.ECDSA(digest_alg_impl))
                elif isinstance(pubkey, dsa.DSAPublicKey):
                    pubkey.verify(raw_signature, signed_info_c14n, digest_alg_impl)
                else:
                    raise InvalidSignature(f"Unsupported public key type: {type(pubkey)}")
