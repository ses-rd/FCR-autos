...
class RegimenesEspecialesElectronico(GeneratedsSuper):
    """Elemento Raiz del Regímenes Especiales Electrónico"""
    subclass = None
    superclass = None
...
class IdDoc(GeneratedsSuper):
    ...
    def __init__(self, TipoeCF, eNCF, FechaVencimientoSecuencia, TipoIngresos, TipoPago, FechaLimitePago=None,
                 TerminoPago=None, TablaFormasPago=None, TipoCuentaPago=None, NumeroCuentaPago=None, BancoPago=None):
        self.original_tagname_ = None
    ...
    def get_TipoCuentaPago(self):
        return self.TipoCuentaPago

    def set_TipoCuentaPago(self, TipoCuentaPago):
        self.TipoCuentaPago = TipoCuentaPago

    def get_NumeroCuentaPago(self):
        return self.NumeroCuentaPago

    def set_NumeroCuentaPago(self, NumeroCuentaPago):
        self.NumeroCuentaPago = NumeroCuentaPago

    def get_BancoPago(self):
        return self.BancoPag

    def set_BancoPago(self, BancoPago):
        self.BancoPago = BancoPago
    ...
    def hasContent_(self):
        if (
                ...
                self.TablaFormasPago is not None or
                self.TipoCuentaPago is not None or
                self.NumeroCuentaPago is not None or
                self.BancoPago is not None
        ):
            return True
        else:
            return False
    ...
    def exportChildren(self, outfile, level, namespace_='', name_='EmisorType', fromsubclass_=False,
        ...
        if self.TipoCuentaPago is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write(bytes(('<TipoCuentaPago>%s</TipoCuentaPago>%s' % (
                self.gds_encode(
                    self.gds_format_string(quote_xml(self.TipoCuentaPago), input_name='TipoCuentaPago')),
                eol_)).encode()))
        if self.NumeroCuentaPago is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write(bytes(('<NumeroCuentaPago>%s</NumeroCuentaPago>%s' % (
                self.gds_encode(
                    self.gds_format_string(quote_xml(self.NumeroCuentaPago), input_name='NumeroCuentaPago')),
                eol_)).encode()))
        if self.BancoPago is not None:
            showIndent(outfile, level, pretty_print)
            outfile.write(bytes(('<BancoPago>%s</BancoPago>%s' % (
                self.gds_encode(
                    self.gds_format_string(quote_xml(self.BancoPago), input_name='BancoPago')),
                eol_)).encode()))
...
class TablaTelefonoEmisor(GeneratedsSuper):
    ...
    def replace_TelefonoEmisor_at(self, index, value):
        self.TelefonoEmisor[index] = value

    def hasContent_(self):
        if (
                self.TelefonoEmisor is not None
        ):
            return True
        else:
            return False
...
class Totales(GeneratedsSuper):
    ...
    def __init__(self, MontoTotal, MontoExento=0.00, MontoImpuestoAdicional=None, ImpuestosAdicionales=None,
                 ValorPagar=None):
        self.original_tagname_ = None
...
class Item(GeneratedsSuper):
    ...
    def __init__(self, NumeroLinea, NombreItem, IndicadorBienoServicio, CantidadItem, PrecioUnitarioItem,
                 ...
                 TablaSubRecargo=None, TablaCodigosItem=None):
        self.original_tagname_ = None
...
class DescuentoORecargo(GeneratedsSuper):
    ...
    def get_MontoDescuentooRecargoOtraMoneda(self):
        return self.MontoDescuentooRecargoOtraMoneda

    def set_MontoDescuentooRecargoOtraMoneda(self, MontoDescuentooRecargoOtraMoneda):
        self.MontoDescuentooRecargoOtraMoneda = MontoDescuentooRecargoOtraMoneda
