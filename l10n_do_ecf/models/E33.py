...
class NotaDebitoElectronica(GeneratedsSuper):
    """Elemento Raiz de la Nota de Débito Electrónica"""
    subclass = None
    superclass = None
...
class Totales(GeneratedsSuper):
    ...
    def __init__(self, MontoTotal, MontoGravadoTotal=0.00, MontoGravadoI1=0.00, MontoGravadoI2=0.00,
                 MontoGravadoI3=0.00, MontoExento=0.00, ITBIS1=0.0, ITBIS2=0.00, ITBIS3=0.00, TotalITBIS=0.00,
...
class DetallesItems(GeneratedsSuper):
    ...
    def export(self, outfile, level, namespace_='', name_='DetallesItemsType', namespacedef_='', pretty_print=True):
        imported_ns_def_ = GenerateDSNamespaceDefs_.get('DetallesItemsType')
        if imported_ns_def_ is not None:
            namespacedef_ = imported_ns_def_
        if pretty_print:
            eol_ = '\n'
        ...
        if self.hasContent_():
            outfile.write(bytes(('>%s' % (eol_,)).encode()))
            self.exportChildren(outfile, level + 1, namespace_='', name_='DetallesItemsType', pretty_print=pretty_print)
            showIndent(outfile, level, pretty_print)
            outfile.write(bytes(('</%s%s>%s' % (namespace_, name_, eol_)).encode()))
        else:
            outfile.write(bytes(('/>%s' % (eol_,)).encode()))
...
class Item(GeneratedsSuper):
    ...
    def __init__(self, NumeroLinea, NombreItem, IndicadorBienoServicio, CantidadItem, PrecioUnitarioItem,
                 ...
                 DescuentoMonto=None, TablaSubDescuento=None, RecargoMonto=None, TablaSubRecargo=None,
